...
export function Rooster({ viewMode: viewModeProp = "week" }: { viewMode?: ViewMode }) {
  ...
  // Day list modal (click on a day column to view shifts for that date)
  const [dayListDialog, setDayListDialog] = useState<{ open: boolean; date?: string }>({ open: false })
  ...
  return (
    <div className="flex flex-col flex-1 min-h-0">
                ...
                {colDates.map(date => {
                  ...
                  return (
                    ...
                    >
                      <button
                        type="button"
                        onClick={() => setDayListDialog({ open: true, date: toDateKey(date) })}
                        className="w-full text-left"
                        aria-label={`View shifts for ${toDateKey(date)}`}
                        aria-expanded={dayListDialog.open}
                      >
      ...
      {/* ── Manage Modal ─────────────────────────────────────────────────────── */}
      {/* ── Day List Modal ─────────────────────────────────────────────────── */}
      <Dialog open={dayListDialog.open} onOpenChange={(v) => setDayListDialog(p => ({ ...p, open: v }))}>
        <DialogContent className="max-w-md rounded-2xl p-0 overflow-hidden gap-0" onOpenAutoFocus={e => e.preventDefault()}>
          <DialogHeader className="px-5 pt-5 pb-4 border-b border-border bg-muted/30">
            <div className="flex items-center gap-3">
              ...
              <div>
                <DialogTitle className="text-sm font-semibold tracking-tight leading-none mb-0.5">Shifts on {dayListDialog.date}</DialogTitle>
                <p className="text-[10px] text-muted-foreground">List of scheduled shifts for the selected date</p>
              </div>
            </div>
          </DialogHeader>
          ...
          <div className="px-5 py-4 flex flex-col gap-3">
            {dayListDialog.date ? (
              (() => {
                const items = shifts.filter(s => s.date === dayListDialog.date).sort((a, b) => (a.startHour ?? 0) - (b.startHour ?? 0))
                if (items.length === 0) return <div className="text-sm text-muted-foreground">No shifts for this date</div>
                return items.map(s => (
                  <div key={s.id} className="flex items-center justify-between gap-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="text-xs text-muted-foreground">{resources.find(r => r.id === s.resourceId)?.name ?? '—'}</div>
                      </div>
                    </div>
                    <div className="text-xs text-muted-foreground">
                      {s.hasTime ? `${formatHour(s.startHour)} — ${formatHour(s.endHour)}` : 'No time'}
                    </div>
                  </div>
                ))
              })()
