...
const BotProfile = lazy(() => import("@/components/BotProfile").then(m => ({ default: m.BotProfile })))
...
const BotReminder = lazy(() => import("@/components/BotReminder").then(m => ({ default: m.BotReminder })))
const WhatsAppMessaging = lazy(() => import("@/components/WhatsAppMessaging").then(m => ({ default: m.WhatsAppMessaging })))
...
function getInitialPageFromHash() {
  ...
  if (page && ["bot-profile", "bot-dashboard", "bot-reminder", "bot-send-chat", "bot-schedule-chat", "bot-delete-message", "bot-contact", "bot-command", "bot-custom-command", "bot-settings", "plano-vm"].includes(page)) {
    return page
  }
...
const QUICK_ACCESS_OPTIONS: QuickAccessOption[] = [
  ...
  { id: "bot-dashboard",    icon: MessageCircle, label: "Account", description: "WhatsApp QR & status", iconClass: "text-green-600" },
]
...
function AppContent() {
  ...
  const handlePageChange = (page: string) => {
    ...
    const trackablePages = ["route-list", "custom", "deliveries", "rooster", "plano-vm", "gallery-album", "gallery-site-images", "settings-profile", "bot-dashboard", "bot-profile", "bot-reminder", "bot-send-chat", "bot-schedule-chat", "bot-delete-message", "bot-contact", "bot-command", "bot-custom-command", "bot-settings"]
  ...
  const renderContent = () => {
    switch (currentPage) {
      ...
      case "bot-contact":
        return <WhatsAppMessaging page={currentPage} />
      case "bot-command":
  ...
  const getPageBreadcrumbs = (): { parent?: { label: string }; current: string } => {
    switch (currentPage) {
      ...
      case "bot-profile":
        return { parent: { label: contentText.breadcrumb.botWhatsApp }, current: contentText.breadcrumb.profile }
      case "bot-reminder":
        return { parent: { label: contentText.breadcrumb.botWhatsApp }, current: "Reminder" }
      case "bot-send-chat":
      ...
      case "bot-delete-message":
        return { parent: { label: contentText.breadcrumb.botWhatsApp }, current: "Deleted Messages" }
      case "bot-contact":
  ...
  return (
    <>
      ...
      <div className="flex flex-col flex-1 min-h-0 overflow-hidden">
        {!isSharedView && (
          <header
            className="sticky top-[env(safe-area-inset-top)] z-20 flex h-11 sm:h-12 shrink-0 items-center gap-2 border-b bg-background/95 px-2.5 sm:px-3 backdrop-blur supports-[backdrop-filter]:bg-background/70"
          >
